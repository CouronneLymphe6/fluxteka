/**
 * Scraper Pipedream — Point d'entrée standalone
 *
 * Usage: npx tsx scripts/scrape-pipedream.ts [maxResults]
 *
 * Crawle les apps Pipedream via l'API publique et les insère en base de données.
 *
 * Source: https://api.pipedream.com/v1/apps
 *
 * Prérequis :
...
async function main() {
  ...
  const maxResults = parseInt(args[0] || '200');

  console.log('═'.repeat(50));
  console.log('  🌀 Fluxteka — Scraper Pipedream');
  console.log('═'.repeat(50));
  ...
  const crawler = new PipedreamCrawler();
    ...
    try {
      ...
      const workflow = await prisma.workflow.create({
        data: {
          ...
          indexing_source: 'pipedream-apps',
          status: 'active',
        },
      });
