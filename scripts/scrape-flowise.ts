/**
 * Scraper Flowise — Point d'entrée standalone
 *
 * Usage: npx tsx scripts/scrape-flowise.ts [maxResults]
 *
 * Crawle les nodes/composants Flowise depuis GitHub et les insère en base.
 *
 * Source: https://api.github.com/repos/FlowiseAI/Flowise/contents/packages/components/nodes
 *
 * Prérequis :
 * - DATABASE_URL dans .env.local
 * - GITHUB_TOKEN dans .env.local (recommandé pour éviter le rate-limit)
 */
...
async function main() {
  ...
  const maxResults = parseInt(args[0] || '80');

  console.log('═'.repeat(50));
  console.log('  🌊 Fluxteka — Scraper Flowise');
  console.log('═'.repeat(50));
  console.log(`Max résultats: ${maxResults}`);
  console.log(`GitHub Token: ${process.env.GITHUB_TOKEN ? '✅ présent' : '⚠️  absent (2s de délai entre chaque node)'}`);
  ...
  const crawler = new FlowiseCrawler();
